...
pub async fn assign_in_tx(
    ...
    if origin != "escalations" {
        if let Some(assigned_mid) = assigned_membership_id {
            let actor_mid: Option<Uuid> = match actor_user_id {
            ...
            let should_emit = match actor_mid {
                Some(amid) => amid != assigned_mid,
                None => true,
            };
