...
- [X] T009 [US1] Implement `TenantContext` + `tenant_context_middleware` in `backend/crates/modules/tenancy/src/lib.rs` (pipeline per research R3): missing header → 400; non-UUID → 400; tenant absent/deleted → 403; platform principal → allow any status; tenant principal → require active membership AND tenant status `active` (suspended → 403 with suspension message, same `unauthorized` code); on denial call `audit::access_denied`; on success attach `TenantContext { tenant_id, tenant_status, principal_kind }` extension and record `tenant.id` on the trace span (depends on T007, T008)
- [X] T010 [US1] Implement `GET /api/v1/tenant` in `backend/crates/modules/tenancy/src/routes.rs` (reads only `TenantContext`, returns `TenantSummary`) and mount it in `backend/crates/server/src/router.rs` inside the tenant-context middleware (platform-scoped routes stay outside per FR-004)
- [X] T011 [US1] Verify: with local Postgres up, `cargo test -p server --test tenancy` — all T006 scenarios green; fix until they pass
...
- [X] T034 Fix remaining `cargo clippy --workspace --all-targets -- -D warnings` failures (5× `needless_borrows_for_generic_args` in `backend/crates/server/tests/tenancy.rs` at lines 637, 668, 716, 740, 768 — `.uri(&format!(...))` should be `.uri(format!(...))`) so the T027 quality gate actually passes; these were previously masked because the `tenancy` crate failed to compile before clippy reached the test file (contradicts)
- [X] T035 Provide a `Store` in `frontend/apps/dashboard/src/app/core/tenant/current-user.service.spec.ts`'s `TestBed.configureTestingModule` (e.g. `provideStore({})` or an equivalent test double) — `CurrentUserService` now injects `Store` directly (added to dispatch `tenantContextActions.setActiveTenant` per T030), but the spec's testing module supplies no `Store` provider, so all three existing tests in this file fail with `NullInjectorError: No provider for Store!` (contradicts)
- [X] T036 Fix the infinite-redirect bug in the "no tenant selected" flow: `/tenant/select` (`frontend/apps/dashboard/src/app/features/tenant/tenant.routes.ts`, added per T032) is nested under the parent route in `frontend/apps/dashboard/src/app/app.routes.ts` that carries `canMatch: [areaAccessGuard]` + `data: { area: 'tenant' }`; Angular evaluates `canMatch` against that parent route for any `/tenant/*` URL, so navigating to `/tenant/select` without an active tenant re-triggers the same guard and redirects back to `/tenant/select`, looping forever and making the prompt page unreachable, per US2/AC4 and the FR-016 edge case (contradicts)
- [X] T037 Add a `006-multi-tenancy-foundation` entry to the Recent Changes section of `CLAUDE.md` per T026/T033 — still not present in the file despite both tasks being marked complete (contradicts)

---

## Phase 9: Convergence

- [X] T038 Add a Taiga-wrapped forbidden-access alert/banner to the dashboard shell that surfaces `userMessageFor()` (from `frontend/apps/dashboard/src/app/core/errors/http-error.mapper.ts`) whenever `apiErrorInterceptor` (`frontend/apps/dashboard/src/app/core/http/api-error.interceptor.ts`) catches a tenant-scoped 403 `unauthorized` response, per FR-017 and research.md R10 — the mapper function is implemented and unit-tested but never invoked by any component/template, so no "you don't have access to this tenant" message currently reaches the user (partial)
- [X] T039 Fix the tenant-select prompt's copy in `frontend/apps/dashboard/src/app/features/tenant/tenant-select/tenant-select.component.ts` (currently reads "Choose a tenant from the tenant switcher in the sidebar") to correctly point users at the topbar, where `TenantSwitcherComponent` (`frontend/apps/dashboard/src/app/layout/topbar/tenant-switcher.component.ts`) actually renders, per US2/AC4 and US3/AC2 (contradicts)
