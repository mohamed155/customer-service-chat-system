...
export interface PlatformAuditLogsState {
...
export const PlatformAuditLogsStore = signalStore(
  withState<PlatformAuditLogsState>({
    entries: [],
    nextCursor: null,
    hasMore: false,
    from: initialFrom(),
  ...
  withMethods((store, api = inject(PlatformAuditLogsApiService)) => {
    const load = rxMethod<{
      from: string;
      to: string;
